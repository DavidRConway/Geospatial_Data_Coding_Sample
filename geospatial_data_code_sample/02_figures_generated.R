...
ggsave('plots/daily_time_series_plots.pdf', width = 12, height = 6)
...
world_temps_joined |>
  ...
  mutate(
    temp_type = recode(
      temp_type,
      annual_pop_weighted_temp = 'Annual Population Weighted Average Temperature by Country',
      annual_area_weighted_temp = 'Annual Area Weighted Average Temperature by Country'
    )
  ) |>
  ggplot() +
...
ggsave('plots/world_map_faceted.pdf', width = 10, height = 12)
...
ggsave('plots/temp_ridge_plot_faceted.pdf', width = 10, height = 12)
